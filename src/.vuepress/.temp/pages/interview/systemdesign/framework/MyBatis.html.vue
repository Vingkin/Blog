<template><div><h2 id="mybatis" tabindex="-1"><a class="header-anchor" href="#mybatis" aria-hidden="true">#</a> Mybatis</h2>
<h3 id="param-requestbody" tabindex="-1"><a class="header-anchor" href="#param-requestbody" aria-hidden="true">#</a> @Param / @RequestBody</h3>
<ol>
<li>发现在dao层方法接口的参数中不用<code v-pre>@Param</code>指定参数名称，在xml中也是可以直接用<code v-pre>#{}</code>来对参数进行引用，查了很多资料解释是idea进行了预处理，所以即使不报错，也需要在dao层参数名称位置指定具体名称，以免其他开发环境下出现bug。</li>
...
<h3 id="和" tabindex="-1"><a class="header-anchor" href="#和" aria-hidden="true">#</a> $和#</h3>
<ol>
<li>
<p><code v-pre>#{}</code>相当于占位符，编译时会进行预处理用<code v-pre>?</code>替换原来的位置。而<code v-pre>${}</code>相当于字符串拼接，不到万不得已不要使用，因为有sql注入的风险。</p>
<p><strong>两种必须要使用<code v-pre>${}</code>的情况：</strong></p>
<ul>
<li>
<p>采用in来进行批量删除时只能采用<code v-pre>${}</code>进行字符串拼接</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token class-name">String</span> ids <span class="token operator">=</span> <span class="token string">"1,2,3"</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><div class="language-sql line-numbers-mode" data-ext="sql"><pre v-pre class="language-sql"><code><span class="token keyword">delete</span> <span class="token keyword">from</span> <span class="token keyword">user</span> <span class="token keyword">where</span> id <span class="token operator">in</span> <span class="token punctuation">(</span>${ids}<span class="token punctuation">)</span> <span class="token comment"># 正确写法</span>
...
<li>
<p>动态设置表名的问题</p>
<div class="language-sql line-numbers-mode" data-ext="sql"><pre v-pre class="language-sql"><code><span class="token keyword">select</span> <span class="token operator">*</span> <span class="token keyword">from</span> <span class="token comment">#{tableName} # 错误的，表名不能有单引号</span>
...
<li>
<p><strong>模糊查询</strong>时如果使用了#进行sql编写，例如<code v-pre>select * from user where name = '%#{name}%'</code>，这种情况占位符<code v-pre>?</code>会被当成字符串来处理，sql会变成<code v-pre>select * from user where name = '%?%'</code>，因此出现错误。</p>
<p><strong>解决方案：</strong></p>
<ul>
<li>
<p><s>采用<code v-pre>${}</code>来拼接sql</s></p>
</li>
<li>
<p>采用mysql自带的concat函数进行拼接</p>
...
<li>
<p><strong>采用双引号进行拼接（最常使用）</strong></p>
...
<li>
<p>如果dao层查询出来的数据没有一个实体类对象与之一一对应，则ResultType可以设置成map，最终将结果相应给前端。</p>
<ul>
<li>
<p><strong>通过association解决多对一映射关系</strong></p>
<blockquote>
<p>一般都可以使用分布查询来解决，先通过user_id查询出User完整信息以及role_id，之后通过role_id查询出Role的完整信息，只需要在service中将查询出来的Role封装到User实体类中即可。</p>
</blockquote>
<p>假设有如下两张表：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code> user表   |    Role表
------------------------
user_id   |  role_id
user_name |  role_name
role_id   |
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>但是User实体类中具有Role实体类：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>通过级联查询将结果直接映射在User实体类中，则需要ResultMap来手动指定映射关系</p>
...
<li>
<p><strong>通过分步查询解决多对一映射关系</strong></p>
<blockquote>
<p>优点是可以开启<strong>延迟加载</strong></p>
<p>通过在配置文件中指定<code v-pre>mybatis.configuration.lazy-loading-enabled=true</code>来手动开启，如果mybatis版本低于3.4.1还需要手动设置<code v-pre>mybatis.configuration.aggressiveLazyLoading=false</code>。</p>
<p>当延迟加载开启后，当我们只查询user信息时，调用<code v-pre>&lt;select id=&quot;getUserAndRole&quot;&gt;</code>则只会执行<code v-pre>select * from user where user_id = #{id}</code>的部分，否则级联部分sql语句也会执行。</p>
<p>延迟加载配置是全局开启的，如果想要细粒度控制则需要设置association的fetchType=&quot;eager&quot;，表明该association不开启延迟加载</p>
</blockquote>
<p>其中<code v-pre>association</code>中的<code v-pre>select</code>对应RoleMapper.java的全类名加上对应的方法名，<code v-pre>column</code>是分步查询的条件</p>
<div class="language-xml line-numbers-mode" data-ext="xml"><pre v-pre class="language-xml"><code><span class="token comment">&lt;!-- RoleMapper.xml --></span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>select</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>通过collection解决一对多映射关系</strong>和<strong>通过分步查询解决一对多映射关系</strong>如上述代码相差不大，故不细加阐述。</p>
</li>
</ul>
</li>
</ol>
<h3 id="mybatis分页操作" tabindex="-1"><a class="header-anchor" href="#mybatis分页操作" aria-hidden="true">#</a> Mybatis分页操作</h3>
<p>Mybatis+PageHelper插件即可实现分页操作</p>
<h3 id="mybatis动态表模式实现" tabindex="-1"><a class="header-anchor" href="#mybatis动态表模式实现" aria-hidden="true">#</a> Mybatis动态表模式实现</h3>
<blockquote>
<p>关键点在于如下拦截器的编写，需要先在业务层中向线程变量存入province属性，然后在拦截其中进行拦截处理。对拦截器代码进行了相关优化，可以自定义属性。比如下面代码，线程变量中如果有province字段，则会与原表名<code v-pre>t_user</code>进行拼接，以<code v-pre>t_user_province</code>格式回调，以达到动态表名的效果，province字段需要从前端传过来。如果未来业务有其他拼接字段，也都可以进行简单修改即可使用。</p>
</blockquote>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">MybatisPlusConfig</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="mybatis-plus" tabindex="-1"><a class="header-anchor" href="#mybatis-plus" aria-hidden="true">#</a> Mybatis Plus</h2>
<h3 id="前端传来不定条件mybatis-plus的解决方案" tabindex="-1"><a class="header-anchor" href="#前端传来不定条件mybatis-plus的解决方案" aria-hidden="true">#</a> 前端传来不定条件Mybatis-Plus的解决方案</h3>
<blockquote>
<p>其wrapper方法中第一个参数为condition，返回true即拼接否则不拼接。</p>
</blockquote>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="函数式sql" tabindex="-1"><a class="header-anchor" href="#函数式sql" aria-hidden="true">#</a> 函数式SQL</h3>
<p>使用<code v-pre>LambdaQueryWrapper</code>和<code v-pre>LambdaUpdateWrapper</code>替代<code v-pre>QueryWrapper</code>和<code v-pre>UpdateWrapper</code>。</p>
<blockquote>
<p>前两者采用函数式方法获取列名，避免列名出错的情况</p>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 使用QueryWrapper的情况</span>
<span class="token class-name">QueryWrapper</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">User</span><span class="token punctuation">></span></span> wrapper <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">QueryWrapper</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token punctuation">></span></span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="mybatis-plus多数据源" tabindex="-1"><a class="header-anchor" href="#mybatis-plus多数据源" aria-hidden="true">#</a> Mybatis-Plus多数据源</h3>
<blockquote>
<p>引入依赖</p>
</blockquote>
...
<p>在业务层实现类方法的类名或方法名上通过<code v-pre>@DS(&quot;datasource&quot;)</code>指定访问数据源。</p>
</blockquote>
...
<blockquote>
<p><a href="https://baomidou.com/pages/ba5b24/" target="_blank" rel="noopener noreferrer">MybatisX<ExternalLinkIcon/></a></p>
</blockquote>
</div></template>
